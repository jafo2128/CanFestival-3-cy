...
unsigned char msg[8] = {0,0,0,0,0,0,0,0};	//TODO: remove this buffer in windows
int i;

    fd0 = (int)fd0 - 2;
    /* checking for input message (blocking) */
    retval = canReadWait((int)fd0, (long*)&m->cob_id, &msg, (unsigned*)&m->len, &flags, &timeStamp, -1);
    /*TODO: remove this buffer in windows*/
    for(i=0; i<8; i++)
    	m->data[i] = msg[i];
    if (retval != canOK)
    {
...
unsigned flags = 0;
     ...
     */ 
    retval = canWriteWait((int)fd0, m->cob_id, m->data, m->len, flags, 10000);
    if (retval != canOK)
    {
...
canStatus retval = canOK;
    ...
    fd0 = canOpenChannel(channel, canWANT_EXTENDED);
    if (fd0 < 0)
    {
     ...
     * the BAUD_* values
     */ 
    retval = canSetBusParams(fd0, baud, 0, 0, 0, 0, 0);
    if (retval != canOK)
    {
    ...
    if (fd0 < 0)
    {
  	    fprintf(stderr, "canOpen_driver (Kvaser) : error opening write channel %d\n", channel);
        return (CAN_HANDLE)(fd0+1);
    }
    canBusOff(fd0);
